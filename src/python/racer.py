...
def say_now(saying):
    ...
    try:
        if saying.startswith("SLEEP"):
            requests.get(
                '{}/say?sleep={}'.format(VOICE_SERVICE_URL, saying.split()[1]))
        else:
    ...
    def web_say(saying):
        try:
            if saying.startswith("SLEEP"):
                requests.get(
                    '%s/say?sleep=%s' %
                    (VOICE_SERVICE_URL, saying.split()[1]))
            else:
    ...
    def update(self, error):
        if self.m_prev_rror != 0x80000000:
            vel = (error * self.m_pgain +
                   (error - self.m_prev_rror) * self.m_dgain) >> 10
            self.m_pos += vel
...
class PID:
    ...
    def update(self, current_value):
        ...
        # print "SP:%2.2f PV:%2.2f Error:%2.2f -> P:%2.2f I:%2.2f D:%2.2f" %
        # (self.set_point, current_value, self.error, P_value, I_value,
        # D_value)
        return PID
...
def ignore(block):
    ...
    lines = (
        block.signature == LEFT_LINE) or (
        block.signature == CENTER_LINE) or (
            block.signature == RIGHT_LINE)
...
class Scene(object):
    ...
    def set_pan_error(self):
        ...
        if len(center) > 1:
            self.m_pan_error = PIXY_X_CENTER - \
                self.m_blockmap[CENTER_LINE][1].x
        else:
            self.m_pan_error = PIXY_X_CENTER - \
                self.m_blockmap[CENTER_LINE][0].x
    ...
    def get_frame(self):
        ...
        if CENTER_LINE in self.m_blockmap:
            for block in self.m_blockmap[CENTER_LINE]:
                ...
                # should look for center of car, not center of pixycam view.
                if block.x > PIXY_X_CENTER:
                    right += 1
                else:
                    left += 1
...
def loop():
    ...
    # print "PID controller: SP=%2.2f PV=%2.2f -> OP=%2.2f" % (0, turn,
    # pid_bias)
    last_turn = turn
...
def drive():
    ...
    # print "Drive: advance=%2.2f, throttle=%2.2f, diff_drive=%2.2f,
    # bias=%2.2f" % (advance, throttle, diff_drive, bias)
    ...
    # print "Drive: syn_drive=%2.2f, left_diff=%2.2f, right_diff=%2.2f" %
    # (syn_drive, left_diff, right_diff)
...
def hailmary(block_count):
    ...
    if bias != 0:
        ...
        pixy.pixy_rcs_set_position(PIXY_RCS_PAN_CHANNEL, PIXY_RCS_CENTER_POS)
        normal_drive = False
    # If hailmary didn't work, hold on to your rosary beads, we're going
    # hunting!
    else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='The Heather RaspberryPi Robot Racer')
    ...
    if args.chatty:
        chatty = True
        # Start thread to listen to say() commands and forward them to the
        # text2speech web service
